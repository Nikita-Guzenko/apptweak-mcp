...
const API_KEY = process.env.APPTWEAK_API_KEY || 'APPTWEAK_API_KEY_REDACTED';
if (!API_KEY) {
...
interface AppTweakResponse<T = any> {
  content: T;
  metadata?: {
    count?: number;
    total?: number;
...
function isRatingBreakdown(obj: any): obj is RatingBreakdown {
  return (
    obj &&
    typeof obj === 'object' &&
    typeof obj['1'] === 'number' &&
...
interface AnalyzeTopKeywordsArgs {
  appIds: string[];
  platform: 'ios' | 'android';
  country?: string;
  limit?: number;
  sortBy?: 'score' | 'volume' | 'rank';
}
...
function isSearchAppArgs(args: unknown): args is SearchAppArgs {
...
function isGetAppDetailsArgs(args: unknown): args is GetAppDetailsArgs {
  if (!args || typeof args !== 'object') return false;
  const a = args as Record<string, unknown>;
  return (
...
function isAnalyzeTopKeywordsArgs(args: unknown): args is AnalyzeTopKeywordsArgs {
  if (!args || typeof args !== 'object') return false;
  const a = args as Record<string, unknown>;
  return (
    Array.isArray(a.appIds) &&
    a.appIds.every(id => typeof id === 'string') &&
    (a.platform === 'ios' || a.platform === 'android') &&
    (a.country === undefined || typeof a.country === 'string') &&
    (a.limit === undefined || typeof a.limit === 'number') &&
    (a.sortBy === undefined || ['score', 'volume', 'rank', 'installs'].includes(a.sortBy as string))
  );
}

function isAnalyzeReviewsArgs(args: unknown): args is AnalyzeReviewsArgs {
  if (!args || typeof args !== 'object') return false;
  const a = args as Record<string, unknown>;
  return (
    typeof a.appId === 'string' &&
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        },
        {
          ...
          description: 'Get keyword suggestions sorted by score (keyword quality), volume (search popularity), or rank (ranking difficulty)',
          inputSchema: {
            type: 'object',
            properties: {
              appIds: {
              ...
              sortBy: {
                ...
                description: 'How to sort keyword suggestions: score (keyword quality), volume (search popularity), rank (ranking difficulty)',
                default: 'score'
              }
            },
      ...
      try {
        switch (request.params.name) {
          case 'search_app': {
            ...
            try {
              const response = await this.axiosInstance.get<AppTweakResponse>(
                `/${platform}/searches.json`,
                {
          ...
          case 'get_app_details': {
            ...
            try {
              const response = await this.axiosInstance.get<AppTweakResponse>(
                `/${platform}/applications/${appId}.json`,
                {
            ...
            for (const appId of appIds) {
              try {
                const response = await this.axiosInstance.get(
                  `/${platform}/applications/${appId}/keywords/top.json`,
                  {
                    params: {
                      country,
                      sort: sortBy
                ...
                // Apply the limit to the API response
                const limitedKeywords = (response.data.content as Keyword[]).slice(0, limit);
          ...
          case 'analyze_reviews': {
            ...
            try {
              ...
              const calculatePercentages = (starCount: Record<string, number>, total: number) => {
              ...
              const calculateSentiment = (starCount: Record<string, number>, total: number) => {
              ...
              const analyzeIssues = (starCount: Record<string, number>, total: number) => {
                const issues = [];
                ...
                const recentRatingDrop = 
                  currentVersion.average < allVersions.average - 0.2;
                if (recentRatingDrop) {
